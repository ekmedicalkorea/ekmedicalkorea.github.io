...
import {
  ...
  Globe, ShoppingBag, Users, Bell, AlertCircle
} from 'lucide-react'

function buildMenuGroups(role) {
  const isAdmin = role === '대표' || role === '관리자'
  const groups = [
    {
      label: '홈',
      items: [
  ...
  if (isAdmin) {
    groups.push({
      label: '홈페이지 관리',
      items: [
        { path: '/intranet/web/orders', label: '주문 관리', icon: ShoppingBag },
        { path: '/intranet/web/partners', label: '거래처 관리', icon: Users },
        { path: '/intranet/web/products', label: '제품 관리', icon: Package },
        { path: '/intranet/web/notices', label: '공지사항', icon: Bell },
        { path: '/intranet/web/claims', label: '클레임/문의', icon: AlertCircle },
      ]
    })
  }
  return groups
...
export default function IntranetLayout() {
  ...
  const menuGroups = buildMenuGroups(intranetUser?.role)

  function handleSignOut() { signOut(); navigate('/intranet') }
    ...
    <div className="flex h-screen bg-[#f0f2f5] font-sans overflow-hidden">
      <aside className={`flex flex-col bg-[#1a2940] transition-all duration-200 flex-shrink-0 ${collapsed ? 'w-14' : 'w-52'}`}>
        <div className={`flex items-center h-14 border-b border-[#243552] flex-shrink-0 ${collapsed ? 'justify-center px-0' : 'px-4 gap-3'}`}>
          <div className="w-7 h-7 bg-[#1976d2] rounded flex items-center justify-center flex-shrink-0"><span className="text-white text-xs font-bold">EK</span></div>
        ...
        {!collapsed && (
          <div className="border-t border-[#243552] p-3 flex-shrink-0">
            <div className="flex items-center gap-2 mb-2">
              <div className="w-7 h-7 bg-[#1976d2] rounded-full flex items-center justify-center flex-shrink-0"><User size={14} className="text-white" /></div>
              <div className="min-w-0"><p className="text-white text-xs font-medium truncate">{intranetUser?.name}</p><p className="text-gray-400 text-xs truncate">{intranetUser?.role}</p></div>
            </div>
            <button onClick={handleSignOut} className="flex items-center gap-2 text-gray-400 hover:text-white text-xs py-1.5 px-2 rounded hover:bg-[#243552] transition-colors"><LogOut size={13} /><span>로그아웃</span></button>
          </div>
        )}
        {collapsed && (
          <div className="border-t border-[#243552] py-2 flex-shrink-0">
            <button onClick={handleSignOut} title="로그아웃" className="flex items-center justify-center h-10 w-full hover:bg-[#243552] transition-colors"><LogOut size={16} className="text-gray-400" /></button>
          </div>
        )}
      </aside>
      <div className="flex-1 flex flex-col overflow-hidden min-w-0">
        <header className="h-14 bg-white border-b border-gray-200 flex items-center px-4 gap-3 flex-shrink-0 shadow-sm">
          <button onClick={() => setCollapsed(v => !v)} className="text-gray-500 hover:text-gray-700 p-1 rounded hover:bg-gray-100 transition-colors">
            {collapsed ? <Menu size={18} /> : <X size={18} />}
          </button>
          <div className="flex-1" />
          <div className="flex items-center gap-2 text-sm text-gray-600"><User size={15} /><span>{intranetUser?.name}</span><span className="text-gray-300">|</span><span className="text-xs text-gray-400">{intranetUser?.role}</span></div>
        </header>
